...
void selectData(const TString conf="samples.conf", // input file
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
	...
	for(int i=0; i<muonArr->GetEntriesFast(); i++){
	  ...
	  if(!isMuonTriggerObj(triggerMenu, mutemp1->hltMatchBits, kFALSE)) continue;
	  ...
	  for(int j=i+1; j<muonArr->GetEntriesFast(); j++){
	    ...
	    if(!isMuonTriggerObj(triggerMenu, mutemp2->hltMatchBits, kFALSE)) continue;
	    ...
	    for(int k=j+1; k<muonArr->GetEntriesFast(); k++){
	      ...
	      if(!isMuonTriggerObj(triggerMenu, mutemp3->hltMatchBits, kFALSE)) continue;
