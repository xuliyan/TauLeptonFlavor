...
void selectGEN(const TString conf="samples.conf", // input file
  ...
  TH1F* hist21 = new TH1F("tau -> 3 muon 21","GEN deltaR All",100,0,5);
  ...
  */
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
    ...
    TString outfilename = ntupDir + TString("/") + snamev[isam] + TString("_Tau3MuMCMatchedCutNoTrigger3T_select.root");
    ...
    for(UInt_t ifile=0; ifile<nfiles; ifile++) {  
      ...
      //for(UInt_t ientry=0; ientry<5000; ientry++) {
        ...
        if(ientry%20000==0) cout << "Processing event " << ientry << ". " << (double)ientry/(double)eventTree->GetEntries()*100 << " percent done with this file." <<count6<<endl;
	...
	for(int i=0; i<genmuonArr.size(); i++){
	  for(int j=0; j<trackermu.size(); j++){	    
	    if(genmuonArr[i]->pdgId * trackermu[j]->q > 0) continue;//same sign
	    Double_t deltaR1 = toolbox::deltaR(genmuonArr[i]->eta, genmuonArr[i]->phi, trackermu[j]->eta, trackermu[j]->phi);
	    deltaR[i][j] = deltaR1;
	  }
	}
	...
	SelMuonArr[0] = (trackermu[rr[0][1]]);
	SelMuonArr[1] = (trackermu[rr[1][1]]);
	SelMuonArr[2] = (trackermu[rr[2][1]]);
	R[0] = min;
	R[1] = submin;
	R[2] = subsubmin;

	
	//Exclude non 3 tracker muon events	
	Bool_t is2global1trk = kTRUE;
	Int_t glocount = 0;
	Int_t trkcount = 0;
	for(int i=0; i<3; i++){
	  if(SelMuonArr[i]->typeBits & baconhep::EMuType::kTracker) trkcount++;
	}
	if(trkcount < 3) {
	  is2global1trk = kFALSE;
	  cout<<SelMuonArr[0]->typeBits<<" "<<SelMuonArr[1]->typeBits<<" "<<SelMuonArr[2]->typeBits<<endl;
	}
	...
	if(deltaR1 < 0.03) passMatch++;
	if(deltaR2 < 0.03) passMatch++;
	if(deltaR3 < 0.03) passMatch++;
	//if(1){
	if(passMatch == 3){
	  count5++;
	  R1->Fill(deltaR1);
	  R2->Fill(deltaR2);
	  R3->Fill(deltaR3);
	  hist20->Fill(deltaR1+deltaR2+deltaR3);
	  hist21->Fill(deltaR1);
	  hist21->Fill(deltaR2);
	  hist21->Fill(deltaR3);
	  //cout<<deltaR1<<" "<<deltaR2<<" "<<deltaR3<<endl;
  ...
  */
  ...
  TCanvas *c6 = new TCanvas("6","6",1200,900);
  xaxis = hist20->GetXaxis();
  yaxis = hist20->GetYaxis();

  xaxis->SetTitle("Sum of deltaR");
  yaxis->SetTitle("Entries");
  yaxis->SetTitleOffset(1.2);
  //yaxis->SetRangeUser(0,1000);
  c6->cd();
  ...
  c6->Print("deltRSum.png");

  TCanvas *c7 = new TCanvas("7","7",1200,900);
  xaxis = hist21->GetXaxis();
  yaxis = hist21->GetYaxis();
